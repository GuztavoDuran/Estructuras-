/*
	Name: Estructura de Tienda
	Author: Gustavo blas Duran
	Date: 20/08/18 10:01
...
int main(){
    double credit = 100;//credito general del usuario
	int option;
	do{
            system("cls");
		menu();
		...
		switch(option){
			...
			case 2:
                ...
                cin >> optionKey;
                emptyPlace = searchSong(optionKey);//posicion de la cancion que se compra
                cout << "posicion de la cancion: " << emptyPlace << endl;
                if(emptyPlace == -1){//se busca la cancion
                    ...
                    if(song[emptyPlace].file.availableProducts <= 0){
                        ...
                        if ((credit - song[emptyPlace].file.price) < 0){
                            ...
                            //funcion para copiar el contenido de una cadena a otra --- strcpy()
                            ...
                            //se resta el credito de la cancion al acredito del usuario
							credit = credit - song[emptyPlace].file.price;
							//se resta un elemento del total de caniones
							song[emptyPlace].file.availableProducts = (song[emptyPlace].file.availableProducts) - (1);
                        }//fin de else, si se llega a comprara la cancion
                    }//fin de else, evalua ael credito
                }//fin del else, si se encutra la cancion
				break;
			case 3:
				system("cls");
				showData();
